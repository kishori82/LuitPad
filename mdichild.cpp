       ...
       // if( ignoreReplaceTest==false ) { ignoreWord->setVisible(false); replaceWord->setVisible(false); }
        ...
        if( validSoftware != 0 )
           getMeanings();
            ...
            meaningText->setText("Available only on the regstered version");
...
bool MdiChild::saveFile(const QString &fileName)
{
  ...
  //  qDebug() << "ignore \n" << ignoreWordString;
    out<<   crypto->encryptToString( Utilities::toLuitPadFormat(document(), toHtml(),ignoreWordString));
...
*/
  //  qDebug() <<"Prefix " <<  toolTipControl->hasAssamesePrefix(this);
