...
void Phonetic::searchRoman2UnicodeTree( TreeNode *curNode, QStack <QString > & seen, QStack<QString> &notSeen, QStack<QString> &word, QList<QWordUnicode> &words, int depth, int traversal_depth) {
    QStack<QString> partword;
...
void Phonetic::clearForNewWord(QString newWord ) {
    ...
    treeNodeList.clear();
     //   qDebug() << " after the foreach and after to set it true " << roman2UnicodeTree;
    roman2UnicodeTree->used = true;

    QHash<QString, TreeNode *>::const_iterator  it;
    for(it = roman2UnicodeTree->links.begin(); it != roman2UnicodeTree->links.end(); ++it) {
       ...
       // qDebug() << "setting keys " << it.key() << " " << roman2UnicodeTree->links.value(it.key())->used;
        if( first == second ) {
...
void Phonetic::phoneticWordChoices(QString newWord, QStringList &wordList, float factor) {
 ...
 //   qDebug() << " choice words";
    ...
    if( newWord.length() == 1) clearForNewWord(newWord);
    ...
    if( notSeen.size() > 0 )
       searchRoman2UnicodeTree(roman2UnicodeTree,  seen, notSeen, word, words, newWord.size(),1);
    ...
    for(int i=0; i < 5000; i++) {
        QStringList charList;
        foreach( QChar c, words.at(i)) charList.append( Phonetic::phoneticEquivString(QString(c).toLower()));
        qDebug() << " word = " << charList;
...
*/
    ...
    foreach(QWordUnicode word, words ) {
        QStringList charList;
        foreach( QChar c, word.word) charList.append( Phonetic::phoneticEquivString(QString(c).toLower()));
        markUsedWord(roman2UnicodeTree, charList,true, newWord.size() );
    }
   ...
   if( newWord.size() < 2) {
    ...
    for(it = allWords->begin(); it != allWords->end() && count < 400; ++it) {
       ...
       if( first == second && wordList.size() < 10) {
          //  qDebug() <<  it.key() << "    "  << it.value() << " LD " << Phonetic::LD(it.value(),newWord) << "<" <<first <<"," << second << ">";
           wordList.append( Utilities::padWithSpaces(Utilities::getUnicode(it.key(),"0x"), maxLenWord) + "\t" + it.value() );
       }
       count++;
...
void Phonetic::createSingleInflections() {
    ...
    for( sit = inflexTypes->value("extra")->begin() ; sit != inflexTypes->value("extra")->end(); ++sit) {
        singleInflexions->insert( sit->key, sit->value);
    }

    for( sit = inflexTypes->value("action")->begin() ; sit != inflexTypes->value("action")->end(); ++sit) {
        singleInflexions->insert( sit->key, sit->value);
    }
