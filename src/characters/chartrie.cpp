...
bool CharTrie::insertWord(TreeNode * charTreeRoot, QList<QString> & charList, QString unicodeChar)
{
...
void  CharTrie::_whatPrefixMatch(TreeNode *root,  QList<QString> &prefix_chars_list, QString &whatMatch)
{
      ...
      if( prefix_chars_list.size()==0 ) return;
...
QString  CharTrie::_getPrefixMatch(TreeNode *root,  QList<QString> &prefix_chars_list)
{
      QString result;
      ...
      if( prefix_chars_list.size()==0 ) return result;
      QString key = prefix_chars_list.at(0);
      prefix_chars_list.removeFirst();
...
bool CharTrie::foundPrefixMatch(TreeNode *root,  QList<QString> &prefix_chars_list)
{
     bool found=false;
     ...
     if( root->fullWord ) {
      ...
      if( prefix_chars_list.size()==0 ) return false;
      QString key = prefix_chars_list.at(0);
      prefix_chars_list.removeFirst();
...
/* get the choices from the sub-tree */
   ...
   // this is the function  the editor calls to predict the choices
  void CharTrie::get_choice( QString prefix, int choicesize, bool autoselect, QList<QKeyValue> & choices ) {
   ...
   //   Utilities::warningBox("hello");
     ...
     // Utilities::warningBox("hello 1");
       get_choice_words( charTree, choices,  prefix_list, wordstack, choicesize,autoselect);
     //   Utilities::warningBox("hello again");
       return;
   }

   // the second function to be called.
   void CharTrie::get_choice_words(TreeNode *root,  QList<QKeyValue> & choices, QList<QString> & words,  QStack<QString> wordstack, int choicesize, bool autoselect)
   {
  ...
  void CharTrie::get_choices_from_subtree(TreeNode *root, QList<QKeyValue> & choices, QStack<QString> & wordstack, int max_choices){
       if (root == NULL)
            return;
...
void CharTrie::printTree( TreeNode *root, QStack<QString> & partword)
 ...
 //          qDebug() << root->links[key]->unicode << "//";
   ...
   void CharTrie::load_saved_maps(QString fileName, QHash<QString,QString> & charMap) {
