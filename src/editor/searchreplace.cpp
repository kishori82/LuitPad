...
void SearchReplaceDialog::nextProcess() {
    ...
    if( cursor.isNull())  cursor = document->textCursor();
    //= document->document();
    QString origWord;
    QRegExp re;
    ...
    if(  !(cursor = document->document()->find(search_box->text(), cursor)).isNull()  )   {
      ...
      //  cursor.insertText( origWord );
        ...
        Utilities::warningBox("Cannot find any more instance of " + search_box->text());
    }
...
void SearchReplaceDialog::replaceAllProcess() {
    ...
    while(  !(cursor = document->document()->find(search_box->text(), cursor)).isNull()  )   {
        cursor.select(QTextCursor::WordUnderCursor);
        origWord = cursor.selectedText();
    ...
    Utilities::warningBox("Replaced " + QString::number(count) + " instances of " + search_box->text()  + " with " + replace_box->text());



    /*
    for( QTextBlock currentBlock = document->document()->firstBlock();\
        currentBlock!=document->document()->lastBlock(); currentBlock= currentBlock.next()) {
