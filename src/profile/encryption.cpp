...
SimpleCrypt *SimpleCrypt::getEncryptor(quint64 key) {
  if (encryptor == NULL) {
    encryptor = new SimpleCrypt(key);
  }
  return encryptor;
}

SimpleCrypt::SimpleCrypt()
    : m_key(0), m_compressionMode(CompressionAuto),
      m_protectionMode(ProtectionChecksum), m_lastError(ErrorNoError) {
  qsrand(uint(QDateTime::currentMSecsSinceEpoch() & 0xFFFF));
}

SimpleCrypt::SimpleCrypt(quint64 key)
    : m_key(key), m_compressionMode(CompressionAuto),
      m_protectionMode(ProtectionChecksum), m_lastError(ErrorNoError) {
  qsrand(uint(QDateTime::currentMSecsSinceEpoch() & 0xFFFF));
  splitKey();
}
...
QString SimpleCrypt::encryptToString(const QString &plaintext) {
  ...
  QString cypherString = QString::fromLatin1(
      cypher.toBase64()); // QString::fromAscii(cypher.toBase64());
  return cypherString;
}
